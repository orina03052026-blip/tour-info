...
- **確認メール（実装済み）**：予約完了時に `sendConfirmationEmail_` がお客様へ**英語の自動返信**を送信
  （件名・本文に予約ID／アクティビティ／日付・時刻／人数、e-bike は身長も記載）。
  Reply-To と BCC は `comecomehimeji@gmail.com`（BCCで予約が入ったことに気づける）。
  メールの差出人は **Web App を実行するアカウント**（＝デプロイした人）になります。
  `comecomehimeji` 名義で送りたい場合は、その人がデプロイするか Gmail の「他のアドレスから送信」を設定。
  ※ MailApp は新しい権限です。コード更新後に関数実行 or 再デプロイすると **「メールを送信」権限の承認**を求められるので許可してください。
- **e-bike の身長入力（実装済み）**：e-bike 2ツアーは予約フォームで **Rider height(s) (cm)** を必須入力にし、
  カレンダー予定・Bookings シート（Notes）・確認メールに記録します（城ガイド/Algueblue は不要）。
